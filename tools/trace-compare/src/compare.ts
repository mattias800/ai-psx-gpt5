import { TraceEvent, TraceStream, ComparisonOptions, DivergenceReport, formatAddress, formatInstruction } from './types.js';

const getCpuEvents = (stream: TraceStream): TraceEvent[] => {
...
export const compareTraces = (
  ...
  const matchedContext: TraceEvent[] = [];
...
const findDifferingRegs = (lhs: TraceEvent, rhs: TraceEvent): string[] => {
  const diffs: string[] = [];
...
const findDifferingMem = (lhs: TraceEvent, rhs: TraceEvent): string[] => {
  const diffs: string[] = [];
...
export const formatDivergenceReport = (report: DivergenceReport): string => {
  const lines: string[] = [];
