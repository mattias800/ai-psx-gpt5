...
const formatReg = (idx: number, regs?: ReadonlyArray<number>): string => {
...
const formatImm16 = (imm: number): string => {
...
const formatMemRef = (base: number, offset: number, regs?: ReadonlyArray<number>): string => {
  const simm = (offset << 16) >> 16;
  const offsetStr = simm === 0 ? '' : formatOffset(offset);
  return `${offsetStr}(${formatReg(base, regs).replace('$', '')})`;
};

const padMnemonic = (mnemonic: string, width: number = 7): string => {
  return mnemonic.padEnd(width, ' ');
};

export const disasmMips = (pc: number, instr: number, regs?: ReadonlyArray<number>): string => {
  ...
  switch (op) {
    case 0x00: // SPECIAL
      switch (fn) {
        case 0x00: // SLL
          if (instr === 0) return padMnemonic('nop');
        ...
        case 0x09: // JALR
          if (rd === 31) return `${padMnemonic('jalr')} ${formatReg(rs, regs)}`;
        ...
        case 0x25: // OR
          if (rt === 0 && rs === 0) return `${padMnemonic('move')} ${formatReg(rd, regs)}, ${formatReg(0, regs)}`;
          if (rt === 0) return `${padMnemonic('move')} ${formatReg(rd, regs)}, ${formatReg(rs, regs)}`;
          if (rs === 0) return `${padMnemonic('move')} ${formatReg(rd, regs)}, ${formatReg(rt, regs)}`;
    ...
    case 0x09: // ADDIU
      if (rs === 0) return `${padMnemonic('li')} ${formatReg(rt, regs)}, ${formatOffset(imm)}`;
    ...
    case 0x10: // COP0
      {
        ...
        switch (copOp) {
          ...
          case 0x10: // COP0 function
            if (fn === 0x10) return padMnemonic('rfe');
...
export const getRegName = (idx: number): string => REG_NAMES[idx] || `r${idx}`;
export const extractUsedRegisters = (instr: number): number[] => {
  const op = (instr >>> 26) & 0x3f;
  const rs = (instr >>> 21) & 0x1f;
  const rt = (instr >>> 16) & 0x1f;
  const rd = (instr >>> 11) & 0x1f;
  ...
  switch (op) {
    case 0x00: // SPECIAL
      switch (fn) {
        ...
        case 0x02: // SRL
        case 0x03: // SRA
          if (instr !== 0) used.push(rt); // source
          break;
        case 0x04: // SLLV
        case 0x06: // SRLV
        case 0x07: // SRAV
    ...
    case 0x10: // COP0
      {
        const copOp = (instr >>> 21) & 0x1f;
        if (copOp === 0x04) used.push(rt); // MTC0
      }
      break;
    ...
    case 0x12: // COP2
      {
        const copOp = (instr >>> 21) & 0x1f;
        if (copOp === 0x04 || copOp === 0x06) used.push(rt); // MTC2/CTC2
      }
      break;
    ...
    case 0x26: // LWR
      ...
      if (op === 0x22 || op === 0x26) used.push(rt); // LWL/LWR use rt as input too
      break;
