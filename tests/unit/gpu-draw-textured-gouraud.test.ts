import { describe, it, expect } from 'vitest';
import { GPU } from '../../packages/emulator-gpu/src/gpu';

describe('GPU textured Gouraud triangle (0x34)', () => {
  const XY = (yy:number,xx:number)=>(((yy & 0x1ff) << 16) | (xx & 0x3ff)) >>> 0;
  const UV = (u:number,v:number)=>(((v & 0xff) << 8) | (u & 0xff)) >>> 0;
  const idx = (yy: number, xx: number) => (yy * 1024 + xx);

  it('modulates white texture by vertex colors at vertices', () => {
    const gpu = new GPU();
    // Upload 2x2 white texture at (10,10)
    const baseX=10, baseY=10;
    gpu.writeGP0((0xa0<<24));
    gpu.writeGP0(XY(baseY, baseX));
    gpu.writeGP0(XY(2,2));
    const WHITE=0x7fff;
    gpu.writeGP0((WHITE<<16)|WHITE);
    gpu.writeGP0((WHITE<<16)|WHITE);
    // Texture base
    gpu.writeGP0(0xe1000000); gpu.writeGP0(XY(baseY, baseX));

    // Textured Gouraud triangle: color1 (red), xy1, uv1, color2 (green), xy2, uv2, color3 (blue), xy3, uv3
    const REDc = 0x0000ff; // R=255,G=0,B=0 packed as 0xRRGGBB
    const GREENc = 0x00ff00;
    const BLUEc = 0xff0000;
    const x=100,y=50;
    gpu.writeGP0((0x34<<24) | (REDc & 0x00ffffff));
    gpu.writeGP0(XY(y,x)); gpu.writeGP0(UV(0,0));
    gpu.writeGP0(GREENc & 0x00ffffff); gpu.writeGP0(XY(y,x+2)); gpu.writeGP0(UV(1,0));
    gpu.writeGP0(BLUEc & 0x00ffffff); gpu.writeGP0(XY(y+2,x)); gpu.writeGP0(UV(0,1));

    const RED=0x7c00, GREEN=0x03e0, BLUE=0x001f;
    expect(gpu.vram[idx(y,x)] & 0xffff).toBe(RED);
    expect(gpu.vram[idx(y,x+2)] & 0xffff).toBe(GREEN);
    expect(gpu.vram[idx(y+2,x)] & 0xffff).toBe(BLUE);
  });
});

