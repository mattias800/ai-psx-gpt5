...
export class GTE {
  ...
  exec(instr: number): void {
    ...
    const clampIR = (v: number, which: 1|2|3): number => {
      if (v > 0x7fff) { flags |= (which===1?SAT_IR1:which===2?SAT_IR2:SAT_IR3); return 0x7fff; }
      if (v < -0x8000) { flags |= (which===1?SAT_IR1:which===2?SAT_IR2:SAT_IR3); return -0x8000; }
      return v | 0;
    };
    if (fn === 0x01) {
      ...
      const ofx = this.cr[13] | 0;
      const ofy = this.cr[14] | 0;
      const projOne = (vx:number, vy:number, vz:number) => {
        const rx = (r[0]*vx + r[1]*vy + r[2]*vz + r[9]) | 0;
        const ry = (r[3]*vx + r[4]*vy + r[5]*vz + r[10]) | 0;
        const rz = (r[6]*vx + r[7]*vy + r[8]*vz + r[11]) | 0;
        const z = rz === 0 ? 1 : rz; if (rz === 0) flags |= DIV_ZERO;
        const sx = ofx + Math.trunc(H * (rx / z));
        const sy = ofy + Math.trunc(H * (ry / z));
        const pack = ((sy & 0xffff) << 16) | (sx & 0xffff);
        this.dr[9]  = clampIR(rx,1);
      ...
      const sxy0 = this.dr[24]>>>0; const x0 = (sxy0 & 0xffff) << 16 >> 16; const y0 = (sxy0 >>> 16) << 16 >> 16;
      const sxy1 = this.dr[25]>>>0; const x1 = (sxy1 & 0xffff) << 16 >> 16; const y1 = (sxy1 >>> 16) << 16 >> 16;
      const sxy2 = this.dr[30]>>>0; const x2 = (sxy2 & 0xffff) << 16 >> 16; const y2 = (sxy2 >>> 16) << 16 >> 16;
      ...
      const packColorFromIR = (ir1:number, ir2:number, ir3:number): number => {
