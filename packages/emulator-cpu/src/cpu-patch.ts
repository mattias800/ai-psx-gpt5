...
export function applySafetyPatch(cpu: R3000A): void {
  ...
  cpu.step = function() {
    ...
    if (this.s.nextPc === 0x64657472) {
      console.warn(`[CPU Safety] Prevented jump to invalid address 0x${this.s.nextPc.toString(16)} (string data)`);
      ...
      console.warn(`  Likely JR instruction to bad address in register`);
