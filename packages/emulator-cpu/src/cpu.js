export function createResetState(entry = 0xBFC00000) {
    return {
...
export class R3000A {
    ...
    enterException(vector, excCode, inDelay = false) {
    ...
    step() {
        ...
        if (this.tracer)
            this.tracer(pc >>> 0, instr >>> 0, this.s);
        ...
        const swl = (addr, val) => {
            ...
            switch (addr & 3) {
                case 0:
                    ...
                    break;
                case 1:
                    nw = ((w & 0x000000ff) | (val & 0xffffff00)) >>> 0;
                    break;
                case 2:
                    nw = ((w & 0x0000ffff) | (val & 0xffff0000)) >>> 0;
                    break;
                case 3:
                    nw = ((w & 0x00ffffff) | (val & 0xff000000)) >>> 0;
                    break;
                default:
                    nw = w;
                    break;
        ...
        const swr = (addr, val) => {
            ...
            switch (addr & 3) {
                case 0:
                    nw = ((w & 0xffffff00) | (val & 0x000000ff)) >>> 0;
                    break;
                case 1:
                    nw = ((w & 0xffff0000) | (val & 0x0000ffff)) >>> 0;
                    break;
                case 2:
                    nw = ((w & 0xff000000) | (val & 0x00ffffff)) >>> 0;
                    break;
                case 3:
                    nw = val >>> 0;
                    break;
        ...
        switch (op) {
            ...
            case 0x04: // BEQ
                if (r[rs] === r[rt])
                    this.s.nextPc = (this.s.pc + ((simm << 2) | 0)) >>> 0;
                break;
            case 0x05: // BNE
                if (r[rs] !== r[rt])
                    this.s.nextPc = (this.s.pc + ((simm << 2) | 0)) >>> 0;
            ...
            case 0x01: { // REGIMM group
                ...
                if (rtField === 0x00) { // BLTZ
                    if (r[rs] < 0)
                        this.s.nextPc = (this.s.pc + offset) >>> 0;
                }
